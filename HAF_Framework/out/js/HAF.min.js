...
}if(e.autoLayout){c(window).off("resize."+e.guid()).on(("resize."+e.guid()),function(){e.messageBus.publish("visual-filtering-layout-change");});}}});function a(e){b.each(e.bindings,function(g,f){var h=/([a-z]+)\s([a-zA-Z0-9\-\.\(\)>]+)/.exec(f);e.$container.off(h[1],h[2]);});}function d(e){b.each(e.bindings,function(g,f){var h=/([a-z]+)\s([a-zA-Z0-9\-\.\(\)>]+)/.exec(f);
e.$container.on(h[1],h[2],function(i){g.call(e,i,this);});});}}(HAF,HAF.DOM));(function(){Handlebars.registerHelper("list",function(b,d){var c="<ul>",e,a;if(b&&b.length){for(e=0,a=b.length;e<a;e++){c+="<li>"+d.fn(b[e])+"</li>";}}return c+"</ul>";});}());(function(a){Handlebars.registerHelper("i18n",function(b,c){c=typeof c==="object"?null:c;
