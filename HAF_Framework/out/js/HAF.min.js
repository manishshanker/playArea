...
function h(i,j){if(b.Messaging&&(j instanceof b.Messaging)){i.parentMessageBus=j;}if(i.injectMessageBus){i.messageBus=(j&&j.inject&&j.inject.messageBus)||new b.Messaging();}var k=(j&&j.inject)||(i.inject&&(typeof i.inject==="function"?i.inject():i.inject));b.each(k,function(l,m){var n=/^controls$|^templates$|^views$|^services$/.exec(m);
...
}return new b.Template("tmpl"+a(j));}var m=c[k];b.Module[m]=b.Module[m]||{};try{return new b.Module[m][a(j)](i.injectMessageBus?i.messageBus:i.parentMessageBus);}catch(l){console.log(l);throw new Error("Dependency instance creation error: ("+k+","+j+" | "+m+"."+(a(j))+")");}}}(HAF));(function(a){a.registerHelper("list",function(d,j,f){var e,h,b,g="",c="";
...
};}(HAF));(function(a,d){var b=function(){this.messageBus=d({});};b.prototype={publish:function(e,f){this.messageBus.trigger(e,[f]);},subscribe:function(g,e,f){var h=this;if(typeof e==="string"){return c(h,f,g,e);}var i={};a.each(e,function(k,j){i[j]=c(h,k,g,j);});return i;},unsubscribe:function(e,f){var g=this;
if(typeof e==="string"){g.messageBus.off(e,f);}else{a.each(e,function(i,h){g.messageBus.off(h,i);});}}};function c(e,i,h,g){var f=function(k,j){i.call(h,j);};e.messageBus.on(g,f);return f;}a.messaging=new b();a.Messaging=b;}(HAF,HAF.DOM));(function(a,f){var h;var c;var e={};var g=false;var b="#/home";
var d=function(){function p(t){b=t;f(window).on("hashchange",q);if(location.hash){q();}else{location.href="#/"+b;}}function q(){c=location.hash;var u=n(c);if(!u){location.href="#/"+b;return;}if(u.page!==h){i(h,u);h=u.page;l(h,u);}var t=n(location.hash);if(!g){if(!m(h)||r(t)){k(t);}}e[h]=u;window.setTimeout(function(){g=false;
},200);}function r(t){return(!e[h]||(t.pageData!==e[t.page].pageData));}function k(t){a.messaging.publish("navigationStateChange:"+h,t);a.messaging.publish("navigationStateChange",t);}function i(u,t){if(u){f("a[href$='#/"+u+"']").removeClass("selected");f("#"+u).removeClass("page-visible");a.messaging.publish("navigationChangedFrom:"+u,t);
}}function l(w,u){f("#"+w).addClass("page-visible");var t=s(w);t.addClass("selected");var v=e[w];if(v){if(v.pageData){location.replace("#/"+w+"/"+v.pageData);if(m(w)){g=true;}}}if(!g){a.messaging.publish("navigationChangedTo:"+h,u);}}function m(u){var t=s(u).attr("data-keep-state");return !(t===false||t==="false");
}function s(t){return f("a[href$='#/"+t+"']");}function n(u){var t=/#\/([a-zA-Z_\-0-9\$]+)(\/(.+))?/.exec(u);if(!t){return null;}return{path:u,page:t[1],pageData:t[3],keepPreviousState:m(t[1])};}function j(t){c=t;location.hash=t;if(window.hasOwnProperty("onhashchange")){q();}}function o(v,w,x,t){var u=new RegExp(("^"+w+"$").replace("?",".").replace(/:[a-zA-Z0-9-_]+/g,function(y){return"([a-zA-Z0-9-_]+)";
