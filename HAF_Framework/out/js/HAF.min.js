...
b=b.charAt(0).toLowerCase()+b.substring(1);}return a.i18nT[b]||"!!!"+d+"!!!";};}(HAF));(function(a,c){var b=function(){var e=c({});function f(h,i){e.trigger(h,[i]);}function d(j,i,k){var h=function(m,l){k.call(j,l);};e.on(i,h);return h;}function g(h,i){e.off(h,i);}return{publish:f,subscribe:d,unsubscribe:g};
};a.messaging=new b();a.Messaging=b;}(HAF,HAF.DOM));(function(a,e){var f;var b;var d={};var c=function(){function l(m){e(window).on("hashchange",j);if(location.hash){j();}else{location.href="#/"+m;}}function j(){b=location.hash;var m=k(b);if(m.page!==f){g(f,m);f=m.page;h(f,m);}d[f]=m;a.messaging.publish("navigationStateChange:"+f,m);
