...
b=b.charAt(0).toLowerCase()+b.substring(1);}return a.i18nT[b]||"!!!"+d+"!!!";};}(HAF));(function(a,c){var b=function(){var e=c(document);function f(h,i){e.trigger(h,[i]);}function d(i,h,j){e.on(h,function(l,k){j.call(i,k);});}function g(h){e.off(h);}return{publish:f,subscribe:d,unsubscribe:g};};a.messaging=new b();
}(HAF,HAF.DOM));(function(a,e){var f;var b;var d={};var c=function(){function l(m){e(window).on("hashchange",j);if(location.hash){j();}else{location.href="#/"+m;}}function j(){b=location.hash;var m=k(b);if(m.page!==f){g(f,m);f=m.page;h(f,m);}d[f]=m;a.messaging.publish("navigationStateChange:"+f,m);a.messaging.publish("navigationStateChange",m);
}function g(n,m){if(n){e("a[href$='#/"+n+"']").removeClass("selected");e("#"+n).removeClass("page-visible");a.messaging.publish("navigationChangedFrom:"+n,m);}}function h(o,m){e("#"+o).addClass("page-visible");e("a[href$='#/"+o+"']").addClass("selected");var n=d[o];if(n){if(n.moduleItem){location.replace("#/"+o+"/"+n.module+"/"+n.moduleItem);
}}a.messaging.publish("navigationChangedTo:"+f,m);}function k(n){var m=n.substr(1).split("/");return{path:n,page:m[1],module:m[2],moduleItem:m[3]};}function i(o,p,q,m){var n=new RegExp(("^"+p+"$").replace("?",".").replace(/:[a-zA-Z0-9-_]+/g,function(r){return"([a-zA-Z0-9-_]+)";})).exec(b.substring(1).replace(/[\/]?$/,""));
if(n){n.splice(0,1);q.apply(o,n);}else{if(m){m.call(o);}}}return{load:l,route:i};};a.navigation=new c();}(HAF,HAF.DOM));(function(b,d){b.templateEngine={getById:f,getByCSSSelector:c,getByURL:a,process:e};function a(h,i){d.get(h,function(j){i(g(j));});}function g(h){return Handlebars.compile(h);}function f(j){var h=d("#"+j);
var i=h.html();h.remove();if(!i){throw new Error("Template id: "+j+", not found!!");}else{return g(i);}}function c(i){var h=d(i).html();if(!h){throw new Error("Template selector: "+i+", not matched any!!");}else{return g(h);}}function e(i,h){return i(h);}}(HAF,HAF.DOM));