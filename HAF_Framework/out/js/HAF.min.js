...
if(this.loadBy===a.Template.LOAD.BY_ID){b[this.guid()]=b[this.guid()]||a.templateEngine.getById(c);}},process:function(c){if(!b[this.guid()]){throw new Error("Template not in cache!!");}return a.templateEngine.process(b[this.guid()],c);},load:function(d){var c=this;if(c.loadBy===a.Template.LOAD.BY_URL){if(b[this.guid()]){d();
...
(function(){Handlebars.registerHelper("list",function(b,d){var c="<ul>",e,a;for(e=0,a=b.length;e<a;e++){c=c+"<li>"+d.fn(b[e])+"</li>";}return c+"</ul>";});}());(function(a){Handlebars.registerHelper("i18n",function(b,c){c=typeof c==="object"?null:c;return new Handlebars.SafeString(a.i18n(b,c));});}(HAF));
(function(a){a.i18n=function(d,c){if(!a.i18nT){throw new Error("No resource bundle included for i18n!!");}if(d===null){return"";}var b;if(c){b=c;}else{b=d.toLowerCase().replace(/ ([a-z])/g,function(e,f){return f.toUpperCase();}).replace(/ /g,"");b=b.charAt(0).toLowerCase()+b.substring(1);}return a.i18nT[b]||"!!!"+d+"!!!";
