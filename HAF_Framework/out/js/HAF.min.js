...
}if(d===null){return"";}var b;if(c){b=c;}else{b=d.toLowerCase().replace(/ ([a-z])/g,function(e,f){return f.toUpperCase();}).replace(/ /g,"");b=b.charAt(0).toLowerCase()+b.substring(1);}return a.i18nT[b]||"!!!"+d+"!!!";};}(HAF));(function(a,d){var b=function(){this.messageBus=d({});};b.prototype={publish:function(e,f){this.messageBus.trigger(e,[f]);
},subscribe:function(g,e,f){var h=this;if(typeof e==="string"){return c(h,f,g,e);}var i={};a.each(e,function(k,j){i[j]=c(h,k,g,j);});return i;},unsubscribe:function(e,f){var g=this;if(typeof e==="string"){g.messageBus.off(e,f);}else{a.each(e,function(i,h){g.messageBus.off(h,i);});}}};function c(e,i,h,g){var f=function(k,j){i.call(h,j);
};e.messageBus.on(g,f);return f;}a.messaging=new b();a.Messaging=b;}(HAF,HAF.DOM));(function(a,e){var f;var b;var d={};var c=function(){function m(n){e(window).on("hashchange",k);if(location.hash){k();}else{location.href="#/"+n;}}function k(){b=location.hash;var n=l(b);if(n.page!==f){g(f,n);f=n.page;
h(f,n);}d[f]=n;a.messaging.publish("navigationStateChange:"+f,n);a.messaging.publish("navigationStateChange",n);}function g(o,n){if(o){e("a[href$='#/"+o+"']").removeClass("selected");e("#"+o).removeClass("page-visible");a.messaging.publish("navigationChangedFrom:"+o,n);}}function h(p,n){e("#"+p).addClass("page-visible");
...
if(window.hasOwnProperty("onhashchange")){k();}}function i(p,q,r,n){var o=new RegExp(("^"+q+"$").replace("?",".").replace(/:[a-zA-Z0-9-_]+/g,function(s){return"([a-zA-Z0-9-_]+)";})).exec(b.substring(1).replace(/[\/]?$/,""));if(o){o.splice(0,1);r.apply(p,o);}else{if(n){n.call(p);}}}return{load:m,route:i,setRoute:j};
