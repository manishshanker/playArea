...
},unbind:function(){a(this);},$el:null,render:function(e){this.$container.html(e);},destroy:function(){this.$container.empty();},hide:function(){var e=this;e.$el.hide();if(e.autoManageEventBind){e.unbind();}if(e.autoLayout){c(window).off("resize."+e.guid());}},show:function(){var e=this;e.$el.show();
if(e.autoManageEventBind){e.bind();}if(e.autoLayout){c(window).off("resize."+e.guid()).on(("resize."+e.guid()),function(){e.layoutChange();});}}});function a(e){b.each(e.bindings,function(g,f){var h=/([a-z]+)\s([a-zA-Z0-9\-\.\(\)>]+)/.exec(f);e.$container.off(h[1],h[2]);});}function d(e){var f=typeof e.bindings==="function"?e.bindings():e.bindings;
