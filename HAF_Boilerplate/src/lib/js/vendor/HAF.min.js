...
b.each(f,function(h,g){var i=/([a-z]+)\s([a-zA-Z0-9\-\.\(\)>]+)/.exec(g);e.$container.on(i[1],i[2],function(j){h.call(e,j,this);});});}}(HAF,HAF.DOM));(function(){Handlebars.registerHelper("list",function(b,d){var c="<ul>",e,a;if(b&&b.length){for(e=0,a=b.length;e<a;e++){c+="<li>"+d.fn(b[e])+"</li>";}}return c+"</ul>";
...
}).replace(/ /g,"");b=b.charAt(0).toLowerCase()+b.substring(1);}return a.i18nT[b]||"!!!"+d+"!!!";};}(HAF));(function(a,c){var b=function(){this.messageBus=c({});};b.prototype={publish:function(d,e){this.messageBus.trigger(d,[e]);},subscribe:function(f,e,g){var d=function(i,h){g.call(f,h);};this.messageBus.on(e,d);
return d;},unsubscribe:function(d,e){this.messageBus.off(d,e);}};a.messaging=new b();a.Messaging=b;}(HAF,HAF.DOM));(function(a,e){var f;var b;var d={};var c=function(){function m(n){e(window).on("hashchange",k);if(location.hash){k();}else{location.href="#/"+n;}}function k(){b=location.hash;var n=l(b);
if(n.page!==f){g(f,n);f=n.page;h(f,n);}d[f]=n;a.messaging.publish("navigationStateChange:"+f,n);a.messaging.publish("navigationStateChange",n);}function g(o,n){if(o){e("a[href$='#/"+o+"']").removeClass("selected");e("#"+o).removeClass("page-visible");a.messaging.publish("navigationChangedFrom:"+o,n);
}}function h(p,n){e("#"+p).addClass("page-visible");e("a[href$='#/"+p+"']").addClass("selected");var o=d[p];if(o){if(o.moduleItem){location.replace("#/"+p+"/"+o.module+"/"+o.moduleItem);}}a.messaging.publish("navigationChangedTo:"+f,n);}function l(o){var n=o.substr(1).split("/");return{path:o,page:n[1],module:n[2],moduleItem:n[3]};
}function j(n){b=n;location.hash=n;if("onhashchange" in window){k();}}function i(p,q,r,n){var o=new RegExp(("^"+q+"$").replace("?",".").replace(/:[a-zA-Z0-9-_]+/g,function(s){return"([a-zA-Z0-9-_]+)";})).exec(b.substring(1).replace(/[\/]?$/,""));if(o){o.splice(0,1);r.apply(p,o);}else{if(n){n.call(p);
}}}return{load:m,route:i,setRoute:j};};a.navigation=new c();}(HAF,HAF.DOM));(function(c,e){c.templateEngine={getById:g,getByCSSSelector:d,getByURL:b,process:f,remove:a};function b(i,j){e.get(i,function(k){j(h(k));});}function h(i){return Handlebars.compile(i);}function a(j){var i=e("#"+j);i.remove();
}function g(k){var i=e("#"+k);var j=i.html();i.remove();if(!j){throw new Error("Template id: "+k+", not found!!");}else{return h(j);}}function d(j){var i=e(j).html();if(!i){throw new Error("Template selector: "+j+", not matched any!!");}else{return h(i);}}function f(j,i){return j(i);}}(HAF,HAF.DOM));
