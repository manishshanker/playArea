...
(function(j){j.dependencyInjector=c;var d={"views":"view","templates":"template","services":"service","controls":"controller"};function c(m,n){if(j.Messaging&&(n instanceof j.Messaging)){m.messageBus=n;}if(m.injectLocalMessageBus){m.localMessageBus=(n&&n.inject&&n.inject.localMessageBus)||new j.Messaging();
}var o=(n&&n.inject)||(m.inject&&(b(m.inject)?m.inject():m.inject));o=i(o);j.each(o,function(p,q){var r=/^controls$|^templates$|^views$|^services$/.exec(q);if(r){if(p instanceof Array){f(p,m,q);}else{if(b(p)){m[q]=p.call(m,m);}else{j.each(p,function(t,s){m[q]=m[q]||{};m[q][s]=m[q][s]={};h(m[q],s,e(t,m,q));
});}}}else{m[q]=e(p,m,q);}});}function f(n,m,o){j.each(n,function(p){m[o]=m[o]||{};if(a(p)){m[o][p]=m[p]||{};h(m[o],p,k(m,o,p));}else{j.each(p,function(r,q){h(m[o],q,e(r,m,o));});}});}function e(n,m,o){return a(n)?k(m,o,n):(b(n)?n():n);}function h(m,n,o){m[n]=o;}function k(m,o,n){if(m.injector){j.Module.dependency=j.Module.dependency||{};
var p=j.Module.dependency[m.injector];if(p){if(p[o][n]){return p[o][n](m);}}}return g(m,o,n);}function l(m){return m.substr(0,1).toUpperCase()+m.substr(1);}function g(m,o,n){if(o==="templates"){j.Module.template=j.Module.template||{};if(j.Module.template[l(n)]){return new j.Module.template[l(n)]();}if(j.Module.template[n]){return j.Module.template[n];
}if(n.indexOf("tmpl!")===0){return j.TemplateByURL(n.substr(5));}return j.TemplateByID("tmpl"+l(n));}var q=d[o];j.Module[q]=j.Module[q]||{};try{return new j.Module[q][l(n)](m.injectLocalMessageBus?m.localMessageBus:m.messageBus);}catch(p){j.errorLogger(p);throw new Error("Dependency instance creation error: ("+o+","+n+" | "+q+"."+(l(n))+")");
}}function i(q){if(a(q)){var p={};var o=q.split(":");var n=o[1];var m=o[0];j.each({"templates":/T/,"views":/V/,"services":/S/},function(s,r){if(s.test(m)){p[r]=[n];}});q=p;}return q;}function b(m){return typeof m==="function";}function a(m){return typeof m==="string";}}(Mettle));(function(a){a.registerHelper("list",function(d,j,f){var e,h,b,g="",c="";
...
});}(Mettle));(function(a){a.i18n=function(d,c){if(!a.i18nT){throw new Error("No resource bundle included for i18n!!");}if(d===null){return"";}var b;if(c){b=c;}else{b=d.toLowerCase().replace(/ ([a-z])/g,function(e,f){return f.toUpperCase();}).replace(/ /g,"");b=b.charAt(0).toLowerCase()+b.substring(1);
}return a.i18nT[b]||"!!!"+d+"!!!";};}(Mettle));(function(f,e){var h;var b;var d={};var g=false;var a="#/home";var c=function(){function p(t){a=t;e(window).on("hashchange",q);if(location.hash){q();}else{location.href="#/"+a;}}function q(){b=location.hash;var u=n(b);if(!u){location.href="#/"+a;return;}if(u.page!==h){i(h,u);
h=u.page;l(h,u);}var t=n(location.hash);if(!g){if(!m(h)||r(t)){k(t);}}d[h]=u;window.setTimeout(function(){g=false;},200);}function r(t){return(!d[h]||(t.pageData!==d[t.page].pageData));}function k(t){f.messaging.publish("navigationStateChange:"+h,t);f.messaging.publish("navigationStateChange",t);}function i(u,t){if(u){e("a[href$='#/"+u+"']").removeClass("selected");
e("#"+u).removeClass("page-visible");f.messaging.publish("navigationChangedFrom:"+u,t);}}function l(w,u){e("#"+w).addClass("page-visible");var t=s(w);t.addClass("selected");var v=d[w];if(v){if(v.pageData){location.replace("#/"+w+"/"+v.pageData);if(m(w)){g=true;}}}if(!g){f.messaging.publish("navigationChangedTo:"+h,u);
}}function m(u){var t=s(u).attr("data-keep-state");return !(t===false||t==="false");}function s(t){return e("a[href$='#/"+t+"']");}function n(u){var t=/#\/([a-zA-Z_\-0-9\$]+)(\/([\w\W]+))?/.exec(u);if(!t){return null;}return{path:u,page:t[1],pageData:t[3],keepPreviousState:m(t[1])};}function j(t){b=t;
location.hash=t;if(window.hasOwnProperty("onhashchange")){q();}}function o(v,w,x,t){var u=new RegExp(("^"+w+"$").replace("?",".").replace(/:[a-zA-Z0-9-_]+/g,function(y){return"([a-zA-Z0-9-_]+)";})).exec(b.substring(1).replace(/[\/]?$/,""));if(u){u.splice(0,1);x.apply(v,u);}else{if(t){t.call(v);}}}return{load:p,route:o,setRoute:j};
};f.navigation=new c();}(Mettle,Mettle.DOM));(function(i,e){i.templateEngine={getById:j,getByCSSSelector:g,getByURL:b,getByString:c,process:a,safeString:d,remove:f};function b(k,l){e.get(k,function(m){l(h(m));});}function d(k){return new Handlebars.SafeString(k);}function h(k){return Handlebars.compile(k);
}function f(l){var k=e("#"+l);k.remove();}function c(k){return h(k);}function j(m){var k=e("#"+m);var l=k.html();k.remove();if(!l){throw new Error("Template id: "+m+", not found!!");}else{return h(l);}}function g(l){var k=e(l).html();if(!k){throw new Error("Template selector: "+l+", not matched any!!");
}else{return h(k);}}function a(l,k){return l(k);}}(Mettle,Mettle.DOM));